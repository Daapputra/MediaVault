...
import { embedVideoMessage, extractVideoMessage } from '../../services/api';
  ...
  const processFile = async () => {
    ...
    try {
      const response = mode === 'embed' 
        ? await embedVideoMessage(file, message, setProgress)
        : await extractVideoMessage(file, setProgress);
        ...
        <div className="space-y-6">
          <div className="glass-panel p-6">
            <h2 className="text-xl font-semibold text-text-primary mb-4">
              {mode === 'embed' ? 'Upload Cover Video (.mp4 / .avi)' : 'Upload Stego Video'}
            </h2>
            ...
            <p className="text-xs text-text-secondary mt-2">
              Uses EOF (End-Of-File) steganography. The hidden message is appended to the video's binary structure without altering any frames.
            </p>
            ...
            {mode === 'embed' && (
              <div className="mt-6">
                ...
                <textarea
                  ...
                  placeholder="Enter the secret message you want to hide in the video..."
            ...
            <div className="mt-6">
              <Button 
                ...
                className="w-full"
              >
                {mode === 'embed' ? 'Hide Message (EOF Stego)' : 'Extract Secret Message'}
              </Button>
            </div>
        ...
        <div className="space-y-6 flex flex-col">
          {mode === 'embed' && file && !result && (
            <div className="glass-panel p-6 flex-1 flex flex-col justify-center items-center">
              <h3 className="text-lg font-medium text-text-primary mb-4 self-start">Cover Video Preview</h3>
              <video
                controls
          ...
          {!result && mode === 'extract' && (
            <div className="glass-panel p-6 bg-secondary/5 border-secondary/20 h-full flex flex-col justify-center">
              ...
              <p className="text-sm text-text-secondary leading-relaxed">
                Upload a video file that previously had a message embedded using this system. The algorithm will search for the hidden EOF payload and decode the XOR-obfuscated message back to plaintext.
              </p>
            </div>
          )}
        </div>
