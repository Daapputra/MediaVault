...
import { compressAudio, decompressAudio } from '../../services/api';
  ...
  const processFile = async () => {
    ...
    try {
      const response = mode === 'compress' 
        ? await compressAudio(file, setProgress)
        : await decompressAudio(file, setProgress);
        ...
        <div className="space-y-6">
          <div className="glass-panel p-6">
            <h2 className="text-xl font-semibold text-text-primary mb-4">
              {mode === 'compress' ? 'Upload Audio (.wav)' : 'Upload .mvault File'}
            </h2>
            
            <FileUploader 
              onFileSelect={handleFileSelect}
              accept={mode === 'compress' ? 'audio/wav' : '.mvault'}
              fileType={mode === 'compress' ? 'audio' : 'file'}
            />
            ...
            <div className="mt-6">
              <Button 
                ...
                className="w-full"
              >
                {mode === 'compress' ? 'Compress Audio (Lossless)' : 'Decompress Audio'}
              </Button>
            </div>
          ...
          <div className="glass-panel p-6 bg-primary/5 border-primary/20">
            ...
            <p className="text-sm text-text-secondary leading-relaxed">
              {mode === 'compress' 
                ? "This uses a custom implementation of Huffman Coding to compress raw PCM audio data. It is 100% lossless, meaning the decompressed audio will sound mathematically identical to the original."
                : "Upload a previously compressed .mvault audio file. The system will decode the Huffman tree and reconstruct the exact original audio in WAV format."}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-6 flex flex-col">
          {mode === 'compress' && file && !result && (
            <div className="glass-panel p-6 flex-1 flex flex-col justify-center items-center">
              <h3 className="text-lg font-medium text-text-primary mb-4 self-start">Original Audio Preview</h3>
