...
def compress_audio():
    if 'file' not in request.files:
        return error_response("No file provided. Please upload an audio file.", 400)
    ...
    try:
        ...
        from modules.compression.audio_codec import compress_audio as do_compress
        result = do_compress(input_path)
...
def decompress_audio():
    ...
    file = request.files['file']
    is_valid, error_msg = validate_file(file, 'audio', allow_compressed=True)
    if not is_valid:
        return error_response(error_msg, 400)
    ...
    try:
        ...
        from modules.compression.audio_codec import decompress_audio as do_decompress
        result = do_decompress(input_path)
...
def compress_video():
    if 'file' not in request.files:
        return error_response("No file provided. Please upload a video file.", 400)
    ...
    try:
        ...
        from modules.compression.video_codec import compress_video as do_compress
        result = do_compress(input_path)
...
def decompress_video():
    ...
    file = request.files['file']
    is_valid, error_msg = validate_file(file, 'video', allow_compressed=True)
    if not is_valid:
        return error_response(error_msg, 400)
    ...
    try:
        ...
        from modules.compression.video_codec import decompress_video as do_decompress
        result = do_decompress(input_path)
        ...
        return success_response(
            ...
            message="Video decompressed successfully"
        )
    except ValueError as e:
        return error_response(str(e), 400)
    except Exception as e:
